...
def main():
    ...
    with st.sidebar:
        ...
        data_status = {
            ...
            "❌ Not loaded",
            "Faculty":
            "✅ Loaded" if st.session_state.data_manager.faculty is not None
            else "❌ Not loaded",
            "Enrollments":
...
def data_management_page():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Courses", "Rooms", "Faculty", "Enrollments"])
    ...
    with tab3:
        ...
        with col1:
            uploaded_file = st.file_uploader("Upload Faculty CSV",
                                             type=['csv'],
                                             key="faculty")
            if uploaded_file is not None:
                try:
...
def view_schedules_page():
    ...
    if not filtered_schedule.empty:
        st.dataframe(filtered_schedule[[
            ...
            'end_time', 'room', 'faculty', 'course_type'
        ]],
    ...
    if st.button("🔍 Check for Conflicts", type="primary"):
        with st.spinner("Analyzing schedule for conflicts..."):
            ...
            if conflicts_df.empty:
                ...
                with col2:
                    st.metric("Faculty Checked", schedule['faculty'].nunique())
                with col3:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    faculty_conflicts = len(conflicts_df[conflicts_df['conflict_type'] == 'Faculty'])
                    st.metric("Faculty Conflicts", faculty_conflicts, delta_color="inverse")
                with col2:
                ...
                # Faculty conflicts
                if faculty_conflicts > 0:
                    st.markdown("### 👨‍🏫 Faculty Conflicts")
                    faculty_conflicts_df = conflicts_df[conflicts_df['conflict_type'] == 'Faculty']
                    for idx, conflict in faculty_conflicts_df.iterrows():
                        with st.expander(f"⚠️ {conflict['resource']} - {conflict['day']}", expanded=True):
                            st.warning(f"""
                            **Faculty:** {conflict['resource']}  
                            **Day:** {conflict['day']}  
                            **Conflict:**
...
def export_results_page():
    ...
    with col2:
        group_by = st.selectbox(
            "Group by", ["None", "Program", "Section", "Faculty", "Room"])
