...
def validate_csv_format(df: pd.DataFrame, data_type: str) -> bool:
    ...
    required_columns = {
        ...
        'faculty': ['faculty_name', 'employment_type', 'max_hours_per_week', 'available_days', 'specialization'],
...
def calculate_faculty_load(schedule_df: pd.DataFrame) -> pd.DataFrame:
    """Calculate faculty workload from schedule"""
    if schedule_df.empty:
        return pd.DataFrame()
    
    faculty_load = schedule_df.groupby('faculty').agg({
        'course_code': 'count',
        'section': 'nunique',
        'start_time': lambda x: len(x)  # Total class sessions
    })
    faculty_load.columns = ['total_classes', 'sections_taught', 'total_sessions']
...
def detect_scheduling_conflicts(schedule_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for faculty in schedule_df['faculty'].unique():
        faculty_schedule = schedule_df[schedule_df['faculty'] == faculty]
        
        for idx1, row1 in faculty_schedule.iterrows():
            for idx2, row2 in faculty_schedule.iterrows():
