...
class ClassScheduler:
    ...
    def load_data(self, courses_df, rooms_df, sections_df):
    ...
    def generate_schedule(self, algorithm="greedy", max_iterations=1000, term_filter=None, allow_conflicts=False):
        ...
        if any(data is None for data in [self.courses, self.rooms, self.sections]):
            raise ValueError("All data (courses, rooms, sections) must be loaded first")
    ...
    def _assign_course_to_section(self, course_row, section_row, schedule_list, 
        ...
        if course_type.lower() == 'both':
            ...
            return self._schedule_single_type_course(
                course_row, section_row, schedule_list,
                 room_schedule, section_schedule,
                 suitable_rooms, allow_conflicts
            )
    ...
    def _schedule_lecture_lab_course(self, course_row, section_row, schedule_list,
        ...
        lecture_assigned = self._schedule_course_component(
            ...
            room_schedule, section_schedule, lecture_rooms, allow_conflicts
        )
        ...
        lab_assigned = self._schedule_course_component(
            ...
            room_schedule, section_schedule, lab_rooms, allow_conflicts
        )
    ...
    def _schedule_single_type_course(self, course_row, section_row, schedule_list,
        ...
        return self._schedule_course_component(
            ...
            room_schedule, section_schedule, suitable_rooms, allow_conflicts
        )
    
    def _schedule_course_component(self, course_row, section_row, component_type, hours,
                                 schedule_list, room_schedule, section_schedule,
                                 suitable_rooms, allow_conflicts):
        ...
        for pattern in time_patterns:
            ...
            for _, room_row in suitable_rooms.iterrows():
                ...
                if self._can_assign_pattern(
                    pattern, room_name, section_name,
                    room_schedule, section_schedule, allow_conflicts
                ):
                    # Make the assignment
                    self._make_assignment(
                        pattern, course_row, section_row, room_name,
                        component_type, schedule_list,
                        room_schedule, section_schedule
                    )
                    return True
    ...
    def _can_assign_pattern(self, pattern, room_name, section_name,
                          room_schedule, section_schedule, allow_conflicts):
    ...
    def _make_assignment(self, pattern, course_row, section_row, room_name,
                       component_type, schedule_list, room_schedule, section_schedule):
    ...
    def _backtracking_scheduling(self, courses_to_schedule, max_iterations, allow_conflicts):
        ...
        # Start backtracking
        if self._backtrack_assign(assignments, 0, schedule_list, 
    ...
    def _backtrack_assign(self, assignments, index, schedule_list,
                         room_schedule, section_schedule, max_iterations, allow_conflicts):
    ...
    def _get_possible_assignments(self, course_row, section_row, 
                                room_schedule, section_schedule, allow_conflicts):
        ...
        for pattern in time_patterns:
                for _, room_row in suitable_rooms.iterrows():
                    ...
                    if allow_conflicts or self._can_assign_pattern(
                        pattern,  room_name, section_row['section_name'],
                        room_schedule, section_schedule, allow_conflicts
                    ):
        ...
        # Sort by preference (e.g., prefer earlier times, etc.)
